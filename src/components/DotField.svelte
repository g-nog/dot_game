<script lang="ts">
  import type { Dot, DotId, GalaxyDuelMatch } from '../domain/galaxy-duel/model';

  import ClaimIllustration from './galaxy/ClaimIllustration.svelte';
  ...
  function finish(event: PointerEvent) {
    ...
    if (
      !nearest &&
      Math.hypot(location.x - points.get(origin)!.x, location.y - points.get(origin)!.y) < 24
    ) {
...
>
  <div class="board-layer">
    ...
    <svg
      ...
      role="application"
      aria-label="Galaxy Duel star field"
      onpointermove={move}
      onpointerup={finish}
