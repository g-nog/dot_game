...
import type { ClaimedTriangle, CommittedLine, Player } from '../../src/domain/galaxy-duel/model';
...
const STORAGE_KEY = `galaxy:${CLASSIC_STORAGE_KEY}`;
const route = '/';
const title = 'Galaxy Duel';
test.describe('Galaxy Duel', () => {
  ...
  test('a restored final line resolves the result and rematch alternates the starter', async ({
    ...
    const from = page.getByRole('button', {
      name: `Connect star ${finalLine.a}`,
    });
    const to = page.getByRole('button', {
      name: `Connect star ${finalLine.b}`,
    });
