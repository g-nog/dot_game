...
import { STORAGE_KEY as CLASSIC_STORAGE_KEY } from '../../src/application/persistence';
...
for (const galaxy of [false, true]) {
  const STORAGE_KEY = galaxy ? `galaxy:${CLASSIC_STORAGE_KEY}` : CLASSIC_STORAGE_KEY;
  const route = galaxy ? '/galaxy-duel/index.html' : '/triangle-duel/index.html';
  const title = galaxy ? 'Galaxy Duel' : 'Triangle Duel';
  test.describe(galaxy ? 'Galaxy Duel' : 'Triangle Duel', () => {
    ...
    test('reduced-motion preference removes motion durations', async ({ page }) => {
      ...
      await expect(page.getByRole('img', { name: /Die result:/ })).toBeVisible();
      expect(
        await page.locator('.die').evaluate((die) => getComputedStyle(die).animationName),
      ).toBe('none');
    });
    ...
    test('corrupt storage is explained and can be discarded', async ({ page }) => {
      ...
      await page.goto(route);
      await expect(
        page.getByRole('heading', { name: 'This match can’t be resumed' }),
      ).toBeVisible();
    ...
    test('a restored final line resolves the result and rematch alternates the starter', async ({
      ...
      const from = page.getByRole('button', {
        name: `${galaxy ? 'Connect star' : 'Start line at'} ${finalLine.a}`,
      });
      const to = page.getByRole('button', {
        name: `${galaxy ? 'Connect star' : 'Start line at'} ${finalLine.b}`,
      });
